...
這個站到處寫著「端到端加密」。那句話是真的,但它有一個**具體的前提**,
值得寫清楚,因為讀者對 E2E 的預期通常比實際保證強。

**成立的部分(不需要信任伺服器)**

伺服器只存密文。訊息用 K_master 派生的金鑰在裝置上加密,Worker、D1 與 R2 拿到的
都是不透明的 bytes。就算整個資料庫外洩,內容也讀不出來 —— 這個保證是**靜態**的,
不依賴伺服器有沒有照規矩跑。

**不成立的部分(需要信任伺服器)**

配對與加好友時,**公鑰是伺服器轉交的**。兩台裝置沒有任何獨立管道確認拿到的公鑰
真的是對方的。所以一個**主動惡意或被入侵的 Worker** 可以在交換的那一刻把公鑰換成
自己的,對雙方各做一半的握手 —— 之後每一則訊息它都解得開、改得掉,而兩邊看到的
一切都正常。

6 位數配對碼**擋不住這件事**:那個碼是伺服器產生、也由伺服器驗證的,所以它不是
兩台裝置之間的共同秘密,不能拿來當 PAKE 把通道綁定住。它防的是「別人猜到你的配對」
(靠三次上限,見 `src/routes/pairing.ts` 開頭),不是「伺服器自己說謊」。

**一句話**:伺服器誠實時,端到端成立;伺服器主動作惡時,它可以中間人。
對「自己部署給自己家人用」的定位,這個交換是合理的 —— 你就是伺服器。
但如果哪天它變成別人代管的服務,這一段就要重新算。

### 要補起來的話:帶外指紋比對

標準解法是讓人繞過伺服器確認一次公鑰:兩台裝置各自把「自己的公鑰 + 對方的公鑰」
雜湊成一組短碼(Signal 的 safety number、WhatsApp 的 security code 都是這個),
使用者用眼睛比一次。伺服器換過公鑰的話,兩邊的短碼一定對不起來。

**成本評估(還沒做,先看評估)**

| | |
|---|---|
| 協定 | 不用改。指紋是既有公鑰的函數,`SHA-256(sort(pubA, pubB))` 取前 60 bit → 12 個數字或 6 個詞 |
| 後端 | 零改動。伺服器完全不參與,它連自己被驗了都不知道 |
| 前端 | 配對完成頁與好友詳情頁各加一個「安全碼」區塊,加一顆「已確認」按鈕把狀態存進本地(不上傳,上傳就沒意義了) |
| 工時 | 估 3–4 小時,其中一半在文案 —— 難的不是算雜湊,是讓人知道**為什麼**要比 |
| UX 代價 | **這是真正的成本**。配對現在是「掃碼 → 打 6 位數 → 完成」,加一步「請兩台裝置一起看這 12 個數字」會讓自用情境變囉嗦。多數人會直接按「已確認」而不看 —— 那樣等於沒做,還多了一層安全感的錯覺 |
| 建議 | 做成**可選**:預設不擋流程,在好友詳情頁常駐顯示安全碼與「已確認」狀態,想比的人自己比。**不要**做成配對必經的一步 |

> 真正該先做的其實是上面那句話本身 —— 把信任邊界寫清楚,比多一個沒人看的短碼有用。
> 指紋比對留在 backlog,等這個站不再只是自己部署給自己用的時候再做。
...
- **通知不直接開外部連結 — 這是刻意的一致性選擇,不是尚未實作。**通知上 URL/檔案訊息的 action 是「查看」:開 BentoDrop 定位到該則訊息(目標一律同源,冷啟動用 `/?m={msgId}` 避開 postMessage 競態),連結由使用者在 app 內親手點。原因有二,分屬不同層、補不起來:(1) `clients.openWindow()` 只接受 http(s),不吃 `intent://`,直接開只能落在 Custom Tab;(2) 通知點擊給的是 SW 開視窗的權限、不是頁面的 transient activation,轉送進 app 再發 `intent://` 一樣被 Chrome 的手勢門檻擋。所以「通知一鍵直達完整瀏覽器」在 Android 無路徑;與其兩條路兩種結果,不如一致。**App 內的「開啟連結」**(真實點擊)在 Android 用 `intent://` 直開**完整 Chrome**(帶 `#` 錨點的連結例外 — intent URI 塞不下 `#` — 維持 Custom Tab 保住錨點);iOS 的 standalone PWA 點外部連結會開 Safari 的檢視器(iOS 提供的唯一路徑),不做分支。另:開啟後網站自己深連結到原生 App(如 PChome)是該網站的行為,與 BentoDrop 無關。
