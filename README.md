...
- Android 裝成 App 後,**通知的「開啟」一鍵開到的是 Custom Tab**(它就是 Chrome — 同一份登入與帳號 — 只是介面嵌在 App 上;右上選單可跳完整 Chrome)。WebAPK 通知沒有被 Android 支援的「直達完整 Chrome」路徑:`intent://` 啟動被 Chrome 的手勢門檻擋下,而通知點擊的手勢傳不進頁面。**App 內的「開啟連結」**(真實點擊)則會用 `intent://` 直接開完整 Chrome;帶 `#` 錨點的連結例外(intent URI 塞不下 `#`),維持 Custom Tab 以保住錨點。另:開啟後網站自己深連結到原生 App(如 PChome)是該網站的行為,與 BentoDrop 無關。
